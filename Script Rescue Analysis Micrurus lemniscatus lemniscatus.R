...
data.pres<-read.csv("suit.pres_lemniscatus.csv")
data.fut45<-read.csv("suit.fut45_lemniscatus.csv")
data.fut85<-read.csv("suit.fut85_lemniscatus.csv")
...
data.occurence<-read.csv("lemniscatus.csv")
...
#juntar occur.data e pres.bio8 com base na localizac??o aproximada
...
#retirar localiza????es repetidas deixando apenas a de menor distancia
occur_order<-arrange(data.join,id,dist)      
occur.final<-distinct(occur_order,id, .keep_all = TRUE) 

#limpar tabela
lemniscatus_occurence<-occur.final[,c(1,2,5,6,7)]  
...
plot(hist.temp.pres, col=rgb(0,0,0,1/2),
     ...
     ylim =c(0,500),
     xlim=c(20,30))
plot(hist.temp.fut45, col=rgb(1,1,1,1/4), add=T)
...
for(i in 1:nsim){
  ...
  sdT<-runif(field45.size,sd1,sd2) 
  g1 <- runif(field45.size,35,45) 
  ...
  g <-round(((45 - 35)*(g.geo - min(g.geo)))/ (max(g.geo)-min(g.geo))) + 40
...
hist(out[,3],xlab="Mean Haldanes in trailing edge",ylab="Number of simulations",nclass=50,col="gray",main="") #Midia de Haldanes no Trailing Edge
t.test(out[,4],out[,3])
mean(out[,3])
min(out[,3])
max(out[,3])
...
plot(map_adap, col = brewer.pal(n = 4, name = "PuBu")) 

#Mapa Adaptation with plasticity RCP4.5  (Trailing)
p.adapP<-(apply(geo.adapP,1,mean))*trailing.edge45 
map_adapP <- rasterFromXYZ(xyz =  cbind(coord,p.adapP) , res = 0.5, crs = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs +towgs84=0,0,0")
plot(map_adapP, col = brewer.pal(n = 4, name = "PuBu")) 
...
###Rescue  Calculation to Futuro RCP8.5
...
plast<-(22.2-26.7)/5 
w2<- (((sd1+sd2)/2)^2)*mean(h2)*50

rho <- 0.9 #autoregressive coefficient
neigh <-10 #neighbour size
longF.85 <-dados.fut45[field85,1]
latF.85 <-dados.fut45[field85,2]
...
for(i in 1:nsim){ 
  ...
  g1.85 <- runif(field85.size,35,45) 
  bp85<-runif(field85.size,0.00001,(plast^2/(mean(sdT))^2))  
  
  Haldane85<-((clima.fut85[field85]-clima.pres[field85])/sdT.85)/g85 
  ...
  g85 <-round(((45 - 35)*(g.geo.85 - min(g.geo.85)))/ (max(g.geo.85)-min(g.geo.85))) + 40
